...
## 현재 상태: 🟢 필수 4대 기능 전부 + 시스템 통합 완료

캡쳐 → 순서 히스토리 → 미리보기 → **편집/페인팅** → 클립보드 복사 → 외부앱 붙여넣기 / 저장까지 **실제 동작**. 빌드 0에러/0경고 · 기동 · **캡쳐→자동선택→편집기(11개 도구 렌더)·설정창·트레이/전역단축키 기동을 런타임 검증 완료**.
...
- 단축키 **재지정** UI(현재 설정창은 단축키 표시 전용) · 트레이 **최소화-상주**(현재 메인창 닫으면 앱 종료)
